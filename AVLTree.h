...
public:
   ...
   void inorderPrize(Node<T,S>* p,T& key1 , T& key2 , double sum) ;
    void inorderFreedem(Node<T,S>* p) ;
    };
...
template<class T, class S>
void AVLTree<T,S>::inorderPrize(Node<T,S>* p,T& key1 , T& key2 , double sum) {
    if(p == nullptr){
        return;
    }
    inorderPrize(p->left,key1 , key2 , sum) ;
    if( key1 <= *(p->key) && *(p->key) < key2){
        *(p->data) += sum ;
    }
    inorderPrize(p->right, key1 , key2 , sum) ;
}

template<class T, class S>
void AVLTree<T,S>::inorderFreedem(Node<T,S>* p) {
    if(p == nullptr){
        return;
    }
    inorderFreedem(p->left) ;
        *(p->data) /= 1 ;
    inorderFreedem(p->right) ;
}
